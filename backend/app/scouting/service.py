...
async def analyze_area(lat: float, lon: float, radius_m: float, mode: str | None,
                        settings: dict, progress_callback=None) -> list[dict]:
    ...
    with Session(engine) as s:
        ...
        corridors = [c.to_dict() for c in s.scalars(select(Corridor).where(Corridor.is_active == 1)).all()]
        sign = [r.to_dict() for r in s.scalars(select(DeerSign).where(DeerSign.is_active == 1)).all()]
        ...
        existing = [r.to_dict() for r in s.scalars(select(ScoutingSuggestion)).all()]
    ...
    with Session(engine) as s:
        if mode == "override":
            for row in s.scalars(select(ScoutingSuggestion)).all():
